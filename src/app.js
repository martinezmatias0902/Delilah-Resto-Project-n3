...
const administrador = (req, res, next) => {
    try {
        ...
        res.status(401).json({ error: 'Error en la validación del usuario' })
    }
};
        ...
        if (nombreUsuario == nombre) {
            ...
            if (contrasenaUsuario == contrasena) {
                ...
                if (bienvenida == 1) {
                    ...
                    res.json({ 'Bienvenido! Usted tiene permisos de CLIENTE': nombre });
                }
            } else {
...
app.get('/pedidos', administrador, (req, res) => {
    try {
...
app.put('/pedidos/:id', administrador, (req, res) => {
    try {
        const { id } = req.params;
...
app.delete('/pedidos/:id', administrador, (req, res) => {
    try {
        const { id } = req.params;
...
app.post('/productos', administrador, (req, res) => {
    try {
...
app.put('/productos/:id', administrador, (req, res) => {
    try {
        const { nombre, stock, precio } = req.body
        const { id } = req.params
...
app.delete('/productos/:id', administrador, (req, res) => {
    try {
        const { id } = req.params;
